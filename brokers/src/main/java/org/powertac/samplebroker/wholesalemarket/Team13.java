...
/*
We build our strategy at top on ZIP given as baseline.
We add one exploration component to ZIP:
 Exploration part: While the ZIP modelling is good, market is still unpredictable and exploration randomly p times can help model this uncertainty.
*/

public class Team13 extends Strategies
{
  ...
  private static Team13 instance = null;
  private Random rand = new Random();

  private Integer numberOfBidsPerAuction = 1;  // define the number of bids a broker places per auction instance

  private Team13(BrokerContext broker, double buyLimitPriceMax, double buyLimitPriceMin, double sellLimitPriceMax, double sellLimitPriceMin, MessageManager messageManager)
  {
  ...
  public static Team13 getInstance(BrokerContext broker, double buyLimitPriceMax, double buyLimitPriceMin, double sellLimitPriceMax, double sellLimitPriceMin, MessageManager messageManager)
  {
    if(instance == null)
    {
      instance = new Team13(broker, buyLimitPriceMax, buyLimitPriceMin, sellLimitPriceMax, sellLimitPriceMin, messageManager);
    }
    return instance;
  }
    ...
    if(remainingTries > 0)
    {
        if(amountNeeded > 0.0)
        {
          ...
          else
          {
            ...
            {
              bid = sellLimitPriceMin + Math.random()*(sellLimitPriceMax - sellLimitPriceMin);
            }
            else
              bid = lpd.limitPrice;
          }
